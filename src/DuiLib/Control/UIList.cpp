...
{
	if (_tcscmp(pstrName, DUI_CTR_LIST) == 0) return static_cast<CListUIEx*>(this);
	if (_tcscmp(pstrName, DUI_CTR_ILIST) == 0) return static_cast<IListUI*>(this);
	if (_tcscmp(pstrName, DUI_CTR_ILISTOWNER) == 0) return static_cast<IListOwnerUI*>(this);
	return CVerticalLayoutUI::GetInterface(pstrName);
}
...
bool CListUIEx::SetMultiItemIndex(CControlUI* pStartControl, int iCount, int iNewStartIndex)
{
	if (pStartControl == NULL || iCount < 0 || iNewStartIndex < 0) return false;
	if (pStartControl->GetInterface(DUI_CTR_LISTHEADER) != NULL) return CVerticalLayoutUI::SetMultiItemIndex(pStartControl, iCount, iNewStartIndex);
	// We also need to recognize header sub-items
...
bool CListUIEx::Remove(CControlUI* pControl, bool bDoNotDestroy)
{
	if (pControl->GetInterface(DUI_CTR_LISTHEADER) != NULL) return CVerticalLayoutUI::Remove(pControl, bDoNotDestroy);
	// We also need to recognize header sub-items
	if (_tcsstr(pControl->GetClass(), DUI_CTR_LISTHEADERITEM) != NULL) return m_pHeader->Remove(pControl, bDoNotDestroy);
...
bool CListUIEx::RemoveAt(int iIndex, bool bDoNotDestroy)
{
	if (!m_pList->RemoveAt(iIndex, bDoNotDestroy)) return false;
...
void CListUIEx::Move(SIZE szOffset, bool bNeedInvalidate)
{
	CVerticalLayoutUI::Move(szOffset, bNeedInvalidate);
	if (!m_pHeader->IsVisible()) m_pHeader->Move(szOffset, false);
}
...
bool CListUIEx::SelectItem(int iIndex, bool bTakeFocus, bool bTriggerEvent)
{
	if (iIndex == m_iCurSel) return true;
...
UINT CListUIEx::GetItemFixedHeight()
{
	return m_ListInfo.uFixedHeight;
}

void CListUIEx::SetItemFixedHeight(UINT nHeight)
{
	m_ListInfo.uFixedHeight = nHeight;
	NeedUpdate();
}

int CListUIEx::GetItemFont(int index)
{
	return m_ListInfo.nFont;
}
...
UINT CListUIEx::GetItemTextStyle()
{
	return m_ListInfo.uTextStyle;
}
...
int CListUIEx::GetItemHLineSize() const
{
	return m_ListInfo.iHLineSize;
}

void CListUIEx::SetItemHLineSize(int iSize)
{
	m_ListInfo.iHLineSize = iSize;
	Invalidate();
}

DWORD CListUIEx::GetItemHLineColor() const
{
	return m_ListInfo.dwHLineColor;
}

void CListUIEx::SetItemHLineColor(DWORD dwLineColor)
{
	m_ListInfo.dwHLineColor = dwLineColor;
	Invalidate();
}

int CListUIEx::GetItemVLineSize() const
{
	return m_ListInfo.iVLineSize;
}

void CListUIEx::SetItemVLineSize(int iSize)
{
	m_ListInfo.iVLineSize = iSize;
	Invalidate();
}

DWORD CListUIEx::GetItemVLineColor() const
{
	return m_ListInfo.dwVLineColor;
}

void CListUIEx::SetItemVLineColor(DWORD dwLineColor)
{
	m_ListInfo.dwVLineColor = dwLineColor;
	Invalidate();
}
...
bool CListUIEx::SortItems(PULVCompareFunc pfnCompare, UINT_PTR dwData)
{
	if (!m_pList) return false;
	int iCurSel = m_iCurSel;
	bool bResult = m_pList->SortItems(pfnCompare, dwData, iCurSel);
...
BOOL CListUIEx::InsertColumn(int nCol,CListHeaderItemUIEx *pHeaderItem)
{
...
BOOL CListUIEx::SetHeaderItemData(int nColumn, CControlUI* pControl)
{
	...
	CListHeaderItemUIEx *pHeaderItem = (CListHeaderItemUIEx *)pHeader->GetItemAt(nColumn);
...
CListHeaderItemUIEx::CListHeaderItemUIEx() : m_bDragable(true), m_uButtonState(0), m_iSepWidth(4),
m_uTextStyle(DT_CENTER | DT_VCENTER | DT_SINGLELINE), m_dwTextColor(0), m_dwSepColor(0),
m_iFont(-1), m_bShowHtml(false)
{
	SetTextPadding(CDuiRect(2, 0, 2, 0));
...
LPCTSTR CListHeaderItemUIEx::GetClass() const
{
	return DUI_CTR_LISTHEADERITEM;
}

LPVOID CListHeaderItemUIEx::GetInterface(LPCTSTR pstrName)
{
	if (_tcscmp(pstrName, DUI_CTR_LISTHEADERITEM) == 0) return this;
	return CContainerUI::GetInterface(pstrName);
}

UINT CListHeaderItemUIEx::GetControlFlags() const
{
	if (IsEnabled() && m_iSepWidth != 0) return UIFLAG_SETCURSOR;
	else return 0;
}

void CListHeaderItemUIEx::SetEnabled(bool bEnable)
{
...
bool CListHeaderItemUIEx::IsDragable() const
{
	return m_bDragable;
}

void CListHeaderItemUIEx::SetDragable(bool bDragable)
{
	m_bDragable = bDragable;
	if (!m_bDragable) m_uButtonState &= ~UISTATE_CAPTURED;
}

DWORD CListHeaderItemUIEx::GetSepWidth() const
{
	return m_iSepWidth;
}

void CListHeaderItemUIEx::SetSepWidth(int iWidth)
{
	m_iSepWidth = iWidth;
}

DWORD CListHeaderItemUIEx::GetTextStyle() const
{
	return m_uTextStyle;
}

void CListHeaderItemUIEx::SetTextStyle(UINT uStyle)
{
	m_uTextStyle = uStyle;
	Invalidate();
}

DWORD CListHeaderItemUIEx::GetTextColor() const
{
	return m_dwTextColor;
}


void CListHeaderItemUIEx::SetTextColor(DWORD dwTextColor)
{
	m_dwTextColor = dwTextColor;
	Invalidate();
}

DWORD CListHeaderItemUIEx::GetSepColor() const
{
	return m_dwSepColor;
}

void CListHeaderItemUIEx::SetSepColor(DWORD dwSepColor)
{
	m_dwSepColor = dwSepColor;
	Invalidate();
}

RECT CListHeaderItemUIEx::GetTextPadding() const
{
	return m_rcTextPadding;
}

void CListHeaderItemUIEx::SetTextPadding(RECT rc)
{
	m_rcTextPadding = rc;
	Invalidate();
}

void CListHeaderItemUIEx::SetFont(int index)
{
	m_iFont = index;
}

bool CListHeaderItemUIEx::IsShowHtml()
{
	return m_bShowHtml;
}

void CListHeaderItemUIEx::SetShowHtml(bool bShowHtml)
{
	if (m_bShowHtml == bShowHtml) return;
...
LPCTSTR CListHeaderItemUIEx::GetNormalImage() const
{
	return m_diNormal.sDrawString;
}

void CListHeaderItemUIEx::SetNormalImage(LPCTSTR pStrImage)
{
	if (m_diNormal.sDrawString == pStrImage && m_diNormal.pImageInfo != NULL) return;
	m_diNormal.Clear();
	m_diNormal.sDrawString = pStrImage;
...
LPCTSTR CListHeaderItemUIEx::GetHotImage() const
{
	return m_diHot.sDrawString;
}

void CListHeaderItemUIEx::SetHotImage(LPCTSTR pStrImage)
{
	if (m_diHot.sDrawString == pStrImage && m_diHot.pImageInfo != NULL) return;
	m_diHot.Clear();
	m_diHot.sDrawString = pStrImage;
...
LPCTSTR CListHeaderItemUIEx::GetPushedImage() const
{
	return m_diPushed.sDrawString;
}

void CListHeaderItemUIEx::SetPushedImage(LPCTSTR pStrImage)
{
	if (m_diPushed.sDrawString == pStrImage && m_diPushed.pImageInfo != NULL) return;
	m_diPushed.Clear();
	m_diPushed.sDrawString = pStrImage;
...
LPCTSTR CListHeaderItemUIEx::GetFocusedImage() const
{
	return m_diFocused.sDrawString;
}

void CListHeaderItemUIEx::SetFocusedImage(LPCTSTR pStrImage)
{
	if (m_diFocused.sDrawString == pStrImage && m_diFocused.pImageInfo != NULL) return;
	m_diFocused.Clear();
	m_diFocused.sDrawString = pStrImage;
...
LPCTSTR CListHeaderItemUIEx::GetSepImage() const
{
	return m_diSep.sDrawString;
}

void CListHeaderItemUIEx::SetSepImage(LPCTSTR pStrImage)
{
	if (m_diSep.sDrawString == pStrImage && m_diSep.pImageInfo != NULL) return;
	m_diSep.Clear();
	m_diSep.sDrawString = pStrImage;
...
void CListHeaderItemUIEx::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	if (_tcscmp(pstrName, _T("dragable")) == 0) SetDragable(_tcscmp(pstrValue, _T("true")) == 0);
	else if (_tcscmp(pstrName, _T("align")) == 0) {
		if (_tcsstr(pstrValue, _T("left")) != NULL) {
...
void CListHeaderItemUIEx::DoEvent(TEventUI& event)
{
	if (!IsMouseEnabled() && event.Type > UIEVENT__MOUSEBEGIN && event.Type < UIEVENT__MOUSEEND) {
		if (m_pParent != NULL) m_pParent->DoEvent(event);
...
SIZE CListHeaderItemUIEx::EstimateSize(SIZE szAvailable)
{
	if (m_cxyFixed.cy == 0) return CDuiSize(m_cxyFixed.cx, m_pManager->GetDefaultFontInfo()->tm.tmHeight + 8);
	return CContainerUI::EstimateSize(szAvailable);
}

RECT CListHeaderItemUIEx::GetThumbRect() const
{
	if (m_iSepWidth >= 0) return CDuiRect(m_rcItem.right - m_iSepWidth, m_rcItem.top, m_rcItem.right, m_rcItem.bottom);
	else return CDuiRect(m_rcItem.left, m_rcItem.top, m_rcItem.left - m_iSepWidth, m_rcItem.bottom);
}

void CListHeaderItemUIEx::PaintStatusImage(HDC hDC)
{
	if (IsFocused()) m_uButtonState |= UISTATE_FOCUSED;
	else m_uButtonState &= ~UISTATE_FOCUSED;
...
void CListHeaderItemUIEx::PaintText(HDC hDC)
{
	if (m_dwTextColor == 0) m_dwTextColor = m_pManager->GetDefaultFontColor();
...
/////////////////////////////////////////////////////////////////////////////////////
//	2017/4/13 扩展的CListContainerElementUI 
//
...
{
	if (_tcscmp(pstrName, DUI_CTR_ILISTITEM) == 0) return static_cast<IListItemUI*>(this);
	if (_tcscmp(pstrName, DUI_CTR_LISTCONTAINERELEMENT) == 0) return static_cast<CListContainerElementUIEx*>(this);
	return CContainerUI::GetInterface(pstrName);
}

IListOwnerUI* CListContainerElementUIEx::GetOwner()
{
	return m_pOwner;
}
...
int CListContainerElementUIEx::GetDrawIndex() const
{
	return m_iDrawIndex;
}

void CListContainerElementUIEx::SetDrawIndex(int iIndex)
{
	m_iDrawIndex = iIndex;
}
...
bool CListContainerElementUIEx::Select(bool bSelect, bool bTriggerEvent)
{
	if (!IsEnabled()) return false;
	if (bSelect == m_bSelected) return true;
	m_bSelected = bSelect;
...
bool CListContainerElementUIEx::IsExpandable() const
{
	return m_bExpandable;
}

void CListContainerElementUIEx::SetExpandable(bool bExpandable)
{
	m_bExpandable = bExpandable;
}
...
bool CListContainerElementUIEx::Expand(bool bExpand)
{
	ASSERT(m_pOwner);
	if (m_pOwner == NULL) return false;
	if (bExpand == m_bExpand) return true;
...
void CListContainerElementUIEx::SetPos(RECT rc, bool bNeedInvalidate)
{
...
bool CListContainerElementUIEx::DoPaint(HDC hDC, const RECT& rcPaint, CControlUI* pStopControl)
{
	DrawItemBk(hDC, m_rcItem);
	return CContainerUI::DoPaint(hDC, rcPaint, pStopControl);
}
