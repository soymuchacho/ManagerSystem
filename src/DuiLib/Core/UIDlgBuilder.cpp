...
CControlUI* CDialogBuilder::Create(IDialogBuilderCallback* pCallback, CPaintManagerUI* pManager, CControlUI* pParent)
{
    ...
    if( pManager ) {
        ...
        for( CMarkupNode node = root.GetChild() ; node.IsValid(); node = node.GetSibling() ) {
            ...
            if( _tcsicmp(pstrClass, _T("Image")) == 0 ) {
			...
			else if (_tcsicmp(pstrClass, _T("NewFont")) == 0) {
				// 2017/4/20 增加自定义字体
				nAttributes = node.GetAttributeCount();
				int id = -1;
				LPCTSTR pFontName = NULL;
				LPCTSTR pFontResource = NULL;
				int size = 12;
				bool bold = false;
				bool underline = false;
				bool italic = false;
				...
				for (int i = 0; i < nAttributes; i++) {
					...
					else if (_tcsicmp(pstrName, _T("resource")) == 0)
					{
						pFontResource = pstrValue;
					}
				}
				if (id >= 0 && pFontName) {
					pManager->AddNewFont(id, pFontResource, pFontName, size, bold, underline, italic, shared);
					if (defaultfont) pManager->SetDefaultFont(pFontName, size, bold, underline, italic, shared);
				}
			}
    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
		...
		//树控件XML解析
            ...
            case 16:
                ...
                break;
            case 20:
				// 2017/4/13 这里使用CListContainerElementUIEx扩展
                if( _tcsicmp(pstrClass, DUI_CTR_LISTCONTAINERELEMENT) == 0 )  pControl = new CListContainerElementUIEx;
                break;
            }
